...
/*
 * Classes below found at http://www.php.net/manual/en/book.xmlrpc.php#99099
 * Modified by Zeryl
 */
...
class PrettyXMLRPC extends PrettyXMLRPCCrumb {
	...
	public function __call($name, $arguments) {
		$this->result = $this->backend->call($name, $arguments);
	}
}
...
class PrettyXMLRPCEpiBackend {
	...
	public function setUrl($url) {
		...
		$port = array_key_exists("port", $url) ? $url["port"] : ($this->scheme == 'https' ? 443 : 80);
		
		$this->url = $scheme . "://" . $url["host"] . ":" . $port . (array_key_exists("path", $url) ? $url["path"] : "") . (array_key_exists("query", $url) ? '?' . $url["query"] : "") . (array_key_exists("fragment", $url) ? "#" . $url["fragment"] : "");
	}
	...
	public function call($name, $arguments) {
		...
		$header = (version_compare(phpversion(), '5.2.8')) > 0 ? $headers : implode("\r\n", $headers);
		
		$context = stream_context_create(array('http' => array('method' => "POST", 'header' => $header, 'content' => $request, 'user_agent' => 'PrettyXMLRPC')));
